...
def _date_to_imap(d: date) -> str:
    """Convert Python date to IMAP date string (DD-Mon-YYYY)."""
...
def fetch_emails_node(state: InboxState) -> InboxState:
    ...
    try:
        ...
        # Search for UNSEEN emails in date range
        status, data = mail.uid(
            "search", None,
            f'(UNSEEN SINCE "{start_str}" BEFORE "{end_str}")'
        )
        if status != "OK":
            state["error"] = "IMAP search failed."
            return state
        ...
        for i in range(0, len(uid_list), batch_size):
            ...
            status2, msg_data = mail.uid("fetch", uid_str, "(RFC822.HEADER UID)")
            if status2 != "OK":
                continue
            for response_part in msg_data:
                if isinstance(response_part, tuple):
                    # Extract uid from envelope
                    uid_raw = response_part[0].decode()
                    uid_match = re.search(r"UID\s+(\d+)", uid_raw, re.IGNORECASE)
...
def categorise_senders_node(state: InboxState) -> InboxState:
    ...
    prompt = f"""You are an email categorisation assistant.
Below is a JSON array of email senders with their display name and email count.
Assign ONE category label to each sender from this list ONLY:
  Newsletters, Promotions, Social Media, Internal Updates, Finance & Billing,
  Travel & Bookings, Spam, Shopping, Job & Recruitment, Personal, Support Tickets, Other
...
"""
    ...
    try:
        ...
        for item in categorised:
            em_key = item.get("email", "").lower()
            if em_key in groups:
                groups[em_key]["category"] = item.get("category", "Other")
    except Exception as exc:
        # On failure, leave all as Uncategorised
        pass
...
st.markdown(
    '<div class="hero-sub">Powered by Gemini · LangGraph · Gmail IMAP — scan, categorise, and act on your inbox in seconds.</div>',
    unsafe_allow_html=True,
)
...
if scan_clicked:
    if start_date > end_date:
        ...
        with st.spinner("🔄 Connecting to Gmail and scanning inbox…"):
...
if st.session_state.scan_done and st.session_state.sender_groups:
    ...
    with c1:
        st.markdown(f'<div class="metric-box"><div class="metric-val">{total_emails}</div><div class="metric-label">Unread Emails</div></div>', unsafe_allow_html=True)
    with c2:
    ...
    with c4:
        st.markdown(f'<div class="metric-box"><div class="metric-val">{vip_count}</div><div class="metric-label">VIP Senders</div></div>', unsafe_allow_html=True)
    ...
    for cat in categories:
        ...
        if cat_senders:  # Only add categories that actually have senders
            category_counts.append({"category": cat, "count": cat_email_count, "senders": cat_senders})
    ...
    for cat_data in sorted_categories:
        ...
        with st.expander(
            f"{emoji} **{cat}** — {cat_sender_count} sender{'s' if cat_sender_count>1 else ''} · {cat_email_count} email{'s' if cat_email_count>1 else ''}",
            expanded=expand_all,
        ):
